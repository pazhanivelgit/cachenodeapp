...
//var vcap_services = require('./vcapServices').VCAP_SERVICES;
...
var redis = require('redis');
var client = redis.createClient(credentials.port, credentials.host, { no_ready_check: true });
...
exports.getCustomerById = function routeGetCustomerById(req, res, next) {
    ...
    if (cus_id) {
        ...
        var fullUrl = req.protocol + '://' + req.get('host') + '/nodeapp/redis/customers/' + req.params.customerId;
        //var instanceId=getInstanceId();
        //var cache_resp=cache.get(id);
        client.get(id, function (err, cache_resp) {
            if (cache_resp) {
                cache_resp = JSON.parse(cache_resp);
                cache_resp.message = 'response from REDIS cache';
                cache_resp.host_url = fullUrl;
                cache_resp.instance_id = instanceId;
                console.log('redis cache response-' + JSON.stringify(cache_resp));
                ...
                customer.find({ customer_id: id }, util.exculdeFields, function (err, customer) {
                    if (err) {
                        ...
                        if (customer.length > 0) {
                            ...
                            addCache(id, JSON.stringify(resp));
